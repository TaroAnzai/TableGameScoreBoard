...
- 未入力セルは `—` にする。
- 入力値は正の整数、負の整数、`0` を許可し、小数は許可しない。
- プレイヤーごとの未入力は許可し、入力済みの点数だけを確定対象にする。未入力を `0` として扱わない。
...
- 参加者一覧を中心にする。
- 大会タブとメンバータブを表示し、メンバーが0人の場合はメンバータブを初期表示する。メンバーが1人以上いる場合は大会タブを初期表示する。
- VIEW以外の権限かつ対象が0件の場合だけ、各タブの空状態文言の下に約2行分の間隔を空け、メンバータブにはメンバーの共通利用と統計集計、大会タブには参加メンバー選択と複数卓登録のインストラクションを表示する。
- 空状態では、編集可能な利用者には既存の追加・作成操作を案内し、VIEW権限では「大会が作成されていません。」「メンバーが登録されていません。」と参照専用の文言を表示する。
- メンバーが0人でも大会作成操作は制限しない。
- 管理操作はまとまりごとに配置する。
  - 参加者
  - 大会
  - 共有
